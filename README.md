...
**Просмотр логов:**
```bash
lugit cicd-logs <репозиторий>
```

Показывает логи выполнения всех хуков в репозитории. Доступно всем пользователям, имеющим доступ к репозиторию (даже без `allow_cicd`).
